...
def station_stats(df):
    ...
    #most_common_start_end_station=df[['Start Station'],['End Station']].value_counts() 
...
def display_data(df):
    while True:
        ...
        if raw == 'yes':
            print(df[start:finish])
            raw_5 = input('Do you want to see more 5 lines of raw data?')
            while raw_5 =='yes':
                start+=5
                finish+=5
                print(df[start:finish])
                raw_5 = input('Do you want to see more 5 lines of raw data?')
        print('Thank you for using the explore bikeshare program.')
        break
